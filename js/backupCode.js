...
//DS tring to push weapson into an array but only given me the length
function storePositions(weapon, playerPosition){
//  tilesWithWeapons.push(weapon);
//  tilesWithPlayers.push(playerPosition);
} 
...
let playerMove = function(event){
  ...
  // check if players turn
    if(isPlayer1Turn && $(value).hasClass('dimmed')){
      ...
      // console.log('from object '+ playerMario.weapon.damage)
        // checkForPlayer(value)
...
function dimThreeTiles(incrementX, incrementY, tileY , tileX ) {
  ...
  let tile = '';
  // hold the current tile position
  // up
  let tilePosition = 0;
  // down
  let tileNewPosition = 0

  // test
  let tileTest = 0

  // players 
  let yxposition = tileY + tileX   

  // loop in 4 direction returns -10, 10, -1, 1
 let numToIncrement = incrementX + incrementY  *10
// console.log('numToIncrement ' + numToIncrement)
// console.log(yxposition)
  ...
  for (let index = 1; index <=3; index++) {
    ...
    // down
    tileNewPosition = yxposition - index * numToIncrement;
   
    //  test
    tileTest = yxposition + index;
    // x axis 
    tileX += incrementX
    ...
    // check if tile is less than 0 or if more rows than 10
    // debugger
